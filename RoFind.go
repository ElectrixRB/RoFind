	...
	if groupinfo.Owner.DisplayName == "" {
		if groupinfo.IsLocked != true {
			if groupinfo.PublicEntryAllowed == true {
				...
				c.Printf("Group: %d is claimable! | Members: %d\n", groupinfo.ID, groupinfo.MemberCount)
				resultfile, err := os.OpenFile("Config/results.txt", os.O_APPEND|os.O_WRONLY, 0644)
				if err != nil {
					fmt.Println(err)
				}
				resultfile.WriteString(fmt.Sprintf("Group: %d is claimable! | Members: %d | https://www.roblox.com/groups/%d\n", groupinfo.ID, groupinfo.MemberCount, groupinfo.ID))
				resultfile.Close()
...
func discordwebhook(groupinfo *GroupInfo) {
	...
	embed.SetFooter("https://discord.gg/5ACyUGK72Y", "https://i.imgur.com/B7BPUl3.png")
DISCORDRESTART:
